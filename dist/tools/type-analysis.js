...
class TypeAnalyzer {
    ...
    assessMigrationComplexity(pythonType, tsMapping) {
        if (tsMapping.confidence === 'low')
            return 'requires-redesign';
        if (pythonType.name === 'Any' || pythonType.name === 'object')
            return 'complex';
        ...
        if (pythonType.generic && pythonType.args) {
            const argComplexities = pythonType.args.map(arg => this.assessMigrationComplexity(arg, this.mapPythonType(arg)));
            if (argComplexities.some(c => c === 'requires-redesign'))
                return 'requires-redesign';
            if (argComplexities.some(c => c === 'complex'))
                return 'complex';
            if (argComplexities.some(c => c === 'moderate'))
                return 'moderate';
        }
        if (tsMapping.confidence === 'medium')
            return 'simple';
        return 'trivial';
    }
    generateConversionNotes(pythonType, tsMapping) {
        const notes = [];
        if (tsMapping.notes) {
    ...
    generateRuntimeConsiderations(pythonType, tsMapping) {
    ...
    generateTestingApproach(pythonType, complexity) {
        const approaches = [];
...
export async function registerTypeAnalysisTool(server) {
    const analyzer = new TypeAnalyzer();
    server.tool("type-analysis", "Analyzes Python types and provides TypeScript mappings with migration guidance, runtime considerations, and testing approaches.", {
