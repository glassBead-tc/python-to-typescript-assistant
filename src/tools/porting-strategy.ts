...
// Types for porting strategy analysis
interface PortingComponent {
  id: string;
  name: string;
  type: "module" | "class" | "function" | "variable" | "import" | "type";
  complexity: "low" | "medium" | "high" | "critical";
  dependencies: string[];
  challenges: string[];
  portingApproach: string;
  estimatedEffort: number; // hours
}

interface PortingPhase {
  phase: number;
  name: string;
  description: string;
...
interface PortingStrategy {
  projectId: string;
  projectName: string;
  totalComponents: number;
  analysisDate: string;
  overallComplexity: "straightforward" | "moderate" | "complex" | "extremely-complex";
  recommendedApproach: "big-bang" | "incremental" | "hybrid" | "rewrite";
  estimatedTotalEffort: number; // hours
  
  // Strategic analysis
  criticalPath: string[];
  majorRisks: string[];
  dependencyBottlenecks: string[];
  recommendedOrder: number[];
  
  // Components and phases
  components: PortingComponent[];
  phases: PortingPhase[];
  ...
  // Progress tracking
  iteration: number;
  stage: "analysis" | "planning" | "execution" | "validation" | "completion";
  nextSteps: string[];
}

class PortingStrategyAnalyzer {
  private strategies: Record<string, PortingStrategy[]> = {};
  private nextComponentId = 1;

  private validatePortingStrategyInput(input: unknown): Partial<PortingStrategy> {
    ...
    return data as Partial<PortingStrategy>;
  }

  private analyzeComplexity(components: PortingComponent[]): PortingStrategy['overallComplexity'] {
    ...
    if (complexityRatio >= 3 || criticalCount > 0) return 'extremely-complex';
    if (complexityRatio >= 2 || highCount >= components.length * 0.3) return 'complex';
    if (complexityRatio >= 1.2) return 'moderate';
    return 'straightforward';
  }

  private recommendApproach(complexity: PortingStrategy['overallComplexity'], 
                           componentCount: number): PortingStrategy['recommendedApproach'] {
    if (complexity === 'extremely-complex' || componentCount > 50) return 'rewrite';
    if (complexity === 'complex' || componentCount > 20) return 'hybrid';
    if (complexity === 'moderate' || componentCount > 10) return 'incremental';
    return 'big-bang';
  }

  private buildDependencyGraph(components: PortingComponent[]): string[] {
  ...
  private getComplexityWeight(complexity: PortingComponent['complexity']): number {
    switch (complexity) {
  ...
  private generatePhases(components: PortingComponent[], approach: PortingStrategy['recommendedApproach']): PortingPhase[] {
    const phases: PortingPhase[] = [];
  ...
  private visualizeStrategy(strategy: PortingStrategy): string {
  ...
  private getComplexityEmoji(complexity: PortingStrategy['overallComplexity']): string {
    switch (complexity) {
  ...
  private getApproachEmoji(approach: PortingStrategy['recommendedApproach']): string {
    switch (approach) {
  ...
  public analyzePortingStrategy(input: unknown): { content: TextContent[]; isError?: boolean } {
    try {
      const validated = this.validatePortingStrategyInput(input);
      
      // Create or update strategy
      const strategy: PortingStrategy = {
        ...
        stage: (validated.stage as PortingStrategy['stage']) || 'analysis',
        nextSteps: []
      };
...
export async function registerPortingStrategyTool(server: McpServer): Promise<void> {
  const analyzer = new PortingStrategyAnalyzer();
  
  server.tool(
    "porting-strategy",
    "Systematic framework for analyzing and planning Python-to-TypeScript porting projects. Provides strategic analysis, risk assessment, and phased migration planning.",
    {
